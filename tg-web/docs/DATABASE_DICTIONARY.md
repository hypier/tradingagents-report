...
本库共 22 张表，承载 TG-web 产品侧持久化，并与 tg-core 共享 `analysis_jobs` 等分析任务表。身份认证由 Clerk 托管，本库只保存本地用户资料与业务数据。支付由 Stripe 托管，本库保存订阅镜像、积分账本与 webhook 幂等日志。

### 1.1 域划分

| 域 | 物理表 | 说明 |
| --- | --- | --- |
| 账户 | `product_users` | Clerk 同步资料与偏好 |
...
| 分析任务 | `analysis_jobs`, `user_report_meta`, `report_share_links` | 与 tg-core 共享的 job 持久化及用户侧报告元数据/分享 |
...
| 软删除 | 当前 schema 无通用 soft-delete；分享链接用 `revoked_at` 表示撤销 |
| 密文 | Stripe / LLM API Key 以 ciphertext 字段存储，不以明文落库 |
| 金额 / 积分 | 美元多为 `numeric`；积分为 `bigint`（`mode: 'number'`） |
| 级联 | 指向 `product_users` / `llm_providers` 的外键多为 `ON DELETE CASCADE` |
...
```mermaid
erDiagram
    product_users ||--o{ billing_subscriptions : has
    product_users ||--|| credit_accounts : owns
    product_users ||--o{ credit_reservations : reserves
    product_users ||--o{ credit_ledger_entries : ledger
    product_users ||--o{ referral_relationships : invitee
    product_users ||--o{ referral_relationships : inviter
    product_users ||--o{ watchlist_items : owns
    product_users ||--o{ user_report_meta : meta
    product_users ||--o{ report_share_links : shares

    analysis_jobs ||--o{ report_share_links : shared_as
    analysis_jobs ||--o{ user_report_meta : annotated
...
```mermaid
erDiagram
    product_users ||--o{ billing_subscriptions : subscriptions
    product_users ||--|| credit_accounts : wallet
    product_users ||--o{ credit_reservations : reservations
    product_users ||--o{ credit_ledger_entries : entries
    product_users ||--o{ referral_relationships : as_invitee
    product_users ||--o{ referral_relationships : as_inviter
...
```mermaid
erDiagram
    product_users ||--o{ user_report_meta : annotates
    product_users ||--o{ report_share_links : creates
    analysis_jobs ||--o{ user_report_meta : "job meta"
    analysis_jobs ||--o{ report_share_links : "share links"
...
flowchart TB
    ...
    subgraph Analysis["分析任务"]
        AJ[analysis_jobs]
        URM[user_report_meta]
        RSL[report_share_links]
    end
...
### 3.1 `product_users`（导出名：`accountUsers`）

Clerk 用户对应的本地账户（偏好设置 + Stripe Customer 关联）。物理表名仍为 `product_users`，以兼容既有迁移。
...
| 名称 | 类型 | 列 |
| --- | --- | --- |
| `product_users_pkey` | PRIMARY KEY | `clerk_user_id` |
| `product_users_stripe_customer_key` | UNIQUE（部分，`stripe_customer_id IS NOT NULL`） | `stripe_customer_id` |
| `product_users_referral_code_key` | UNIQUE | `referral_code` |
...
| `clerk_user_id` | `text` | N | — | **FK → product_users**（CASCADE） |
...
| 字段 | 类型 | 空 | 默认 | 说明 |
| --- | --- | --- | --- | --- |
| `clerk_user_id` | `text` | N | — | **PK / FK → product_users**（CASCADE）。每用户一个积分钱包 |
...
| 字段 | 类型 | 空 | 默认 | 说明 |
| --- | --- | --- | --- | --- |
| `invitee_clerk_user_id` | `text` | N | — | **PK / FK → product_users**（CASCADE）。被邀请人 |
| `inviter_clerk_user_id` | `text` | N | — | **FK → product_users**（CASCADE）。邀请人 |
...
### 3.17 `user_report_meta`

每用户报告元数据（归档等）。收藏语义已统一到自选股；`is_favorite` 仍保留兼容。

| 字段 | 类型 | 空 | 默认 | 说明 |
| --- | --- | --- | --- | --- |
| `clerk_user_id` | `text` | N | — | **复合 PK / FK → product_users**（CASCADE） |
| `analysis_job_id` | `uuid` | N | — | **复合 PK**。对应 `analysis_jobs.id`（schema 未声明 FK） |
| `is_favorite` | `integer` | N | `0` | 是否收藏（兼容字段） |
| `is_archived` | `integer` | N | `0` | 是否归档 |
| `notes` | `text` | Y | — | 用户备注 |
| `updated_at` | `timestamptz` | N | `now()` | 更新时间 |
...
| 名称 | 类型 | 列 |
| --- | --- | --- |
| `user_report_meta_pkey` | PRIMARY KEY | `(clerk_user_id, analysis_job_id)` |
| `user_report_meta_user_favorite_idx` | INDEX | `(clerk_user_id, is_favorite)` |
| `user_report_meta_user_archived_idx` | INDEX | `(clerk_user_id, is_archived)` |

---

### 3.18 `report_share_links`
...
| --- | --- | --- | --- | --- |
| `id` | `uuid` | N | `gen_random_uuid()` | **PK** |
| `token` | `text` | N | — | 访问令牌（唯一） |
| `analysis_job_id` | `uuid` | N | — | **FK → analysis_jobs.id**（CASCADE） |
| `clerk_user_id` | `text` | N | — | **FK → product_users**（CASCADE）。创建者 |
| `expires_at` | `timestamptz` | N | — | 过期时间 |
| `revoked_at` | `timestamptz` | Y | — | 撤销时间；非空表示已撤销 |
| `max_views` | `integer` | Y | — | 最大查看次数；null 表示不限 |
| `view_count` | `integer` | N | `0` | 已查看次数 |
| `created_at` | `timestamptz` | N | `now()` | 创建时间 |
...
| 名称 | 类型 | 列 |
| --- | --- | --- |
| `report_share_links_pkey` | PRIMARY KEY | `id` |
| `report_share_links_token_key` | UNIQUE | `token` |
| `report_share_links_job_idx` | INDEX | `analysis_job_id` |

---

### 3.19 `product_settings`
...
| --- | --- | --- | --- | --- |
| `billing_subscriptions` | `clerk_user_id` | `product_users` | `clerk_user_id` | CASCADE |
| `credit_accounts` | `clerk_user_id` | `product_users` | `clerk_user_id` | CASCADE |
| `referral_relationships` | `invitee_clerk_user_id` | `product_users` | `clerk_user_id` | CASCADE |
| `referral_relationships` | `inviter_clerk_user_id` | `product_users` | `clerk_user_id` | CASCADE |
| `credit_reservations` | `clerk_user_id` | `product_users` | `clerk_user_id` | CASCADE |
| `credit_ledger_entries` | `clerk_user_id` | `product_users` | `clerk_user_id` | CASCADE |
| `llm_models` | `provider_id` | `llm_providers` | `id` | CASCADE |
| `watchlist_items` | `clerk_user_id` | `product_users` | `clerk_user_id` | CASCADE |
| `user_report_meta` | `clerk_user_id` | `product_users` | `clerk_user_id` | CASCADE |
| `report_share_links` | `analysis_job_id` | `analysis_jobs` | `id` | CASCADE |
| `report_share_links` | `clerk_user_id` | `product_users` | `clerk_user_id` | CASCADE |
...
| `user_report_meta` | `analysis_job_id` | `analysis_jobs.id` | 用户报告元数据；复合主键组成部分 |
...
| 表.字段 | 取值 |
| --- | --- |
| `product_users.interface_language` | `en` \| `zh-CN`（约定） |
| `product_users.default_market` | `US` \| `HK` \| `CN` \| `CRYPTO`（约定） |
