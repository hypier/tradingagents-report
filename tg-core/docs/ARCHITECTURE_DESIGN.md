...
> 主要入口：`tradingagents` CLI、`TradingAgentsGraph` Python API
...
```mermaid
flowchart TB
    User["用户或上层应用"] --> CLI["CLI: cli.main"]
    User --> API["Python API: main.py / TradingAgentsGraph"]

    CLI --> Config["配置构建\nDEFAULT_CONFIG + 交互选择 + 环境变量"]
    API --> Config
    ...
    Tools --> DataRouter["dataflows.interface.route_to_vendor"]
...
LLM Provider 由 `TradingAgentsGraph` 初始化后注入 Agent 工厂。Agent 不直接感知当前使用哪家模型供应商。

## 5. 核心模块职责

| 模块 | 责任 | 关键文件 |
|---|---|---|
| CLI 入口 | 交互式收集参数、实时展示进度、流式消费 graph、保存报告 | `cli/main.py` |
...
API 路径使用 `TradingAgentsGraph.propagate()`，会处理记忆回填、状态日志、checkpoint 编译与清理、最终评级提取。
...
| Sentiment Analyst | ticker、日期、近 7 天窗口 | Yahoo 新闻、StockTwits、Reddit | `sentiment_report` |
...
供应商路由不会静默加入未配置供应商。若要启用 fallback，需要显式配置链路，例如：
...
| 日期正确性 | yfinance inclusive end、新闻 look-ahead 过滤、陈旧 OHLCV 拒绝 |
...
1. CLI 和 Python API 是两条执行路径。API 的 `propagate()` 包含完整记忆、状态日志、checkpoint 生命周期。CLI 为了实时展示直接调用 `graph.graph.stream()`，并自行合并 chunk，因此部分 API 生命周期能力没有完全复用。
...
| 优先级 | 建议 | 目标 |
|---|---|---|
| P0 | 合并 CLI 与 API 的共享 runner | 让 checkpoint、memory、状态日志在所有入口一致 |
