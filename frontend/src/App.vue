    ...
    <el-drawer v-model="drawerVisible" :size="drawerSize" destroy-on-close class="detail-drawer">
      <template #header
        ><div class="drawer-heading">
          <span>分析详情</span><strong>{{ detail?.ticker || selectedJob?.ticker }}</strong>
        </div></template
      >
      <div v-loading="detailLoading" class="detail-content">
        <template v-if="detail">
          <section class="detail-status">
            ...
            <div>
              <span>分析日期</span><strong>{{ detail.trade_date || '-' }}</strong>
            </div>
            <div>
              <span>Token</span><strong>{{ Number(detail.usage?.tokens || 0).toLocaleString() }}</strong>
            </div>
            <div>
              <span>成本</span><strong>${{ Number(detail.cost?.usd || 0).toFixed(4) }}</strong>
            </div>
          </section>
          <el-alert
            v-if="detail.error"
          ...
          >
            <div>
              <strong>{{ detail.progress?.current_step || '等待开始' }}</strong
              ><span>{{ detail.progress?.percent || 0 }}%</span>
            </div>
            <el-progress :percentage="detail.progress?.percent || 0" :stroke-width="10" />
          </section>
          <section v-if="detail.status === 'succeeded'" class="decision-panel">
            ...
            <p class="decision-copy">
              {{ detail.decision.reasoning || '分析已完成。' }}
            </p>
