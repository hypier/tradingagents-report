...
| 持久化 | Markdown、JSON、SQLite checkpoint、PostgreSQL job 与模型价格缓存 |
...
api
  -> application（创建与执行 job）
  -> infrastructure（服务启动、恢复、任务查询与价格缓存）

application
  -> infrastructure（job 状态与定价协调）
```

`api/` 与 `cli/` 是并列适配层，保持直接函数调用：不引入 ports、Repository、依赖注入容器、ORM、Alembic 或外部队列。`application/` 直接调用 `infrastructure/` 完成 job 状态和定价协调；`api/app.py` 仅为服务生命周期和只读查询直接调用基础设施函数。LLM Provider 由 `TradingAgentsGraph` 初始化，并在构造 Agent 时作为普通参数传入；Agent 不直接感知当前使用哪家模型供应商。

## 5. 核心模块职责

| 模块 | 责任 | 关键文件 |
|---|---|---|
| HTTP 适配 | 路由、鉴权、响应、服务生命周期和任务查询 | `api/app.py`（`api.app:app`） |
...
2. 根据选择构建运行配置。
3. 构造 `application.analysis.AnalysisCommand`。
4. 初始化 Rich Live UI。
5. 通过 `application.analysis.run_analysis()` 启动共享分析用例，并传入 `TokenUsageCallback`。
6. 消费 `AnalysisEvent.state_update`，实时更新 Agent 状态、报告片段、LLM 调用统计和工具调用统计。
7. 使用 `AnalysisResult.final_state` 保存或展示最终报告。
...
`application.analysis.run_analysis()` 是 CLI 和 HTTP job 共用的执行入口。它构造 `TradingAgentsGraph`，将状态增量转换为 `AnalysisEvent` 后调用可选回调，再委托 `TradingAgentsGraph.propagate()` 处理记忆回填、状态日志、checkpoint 编译与清理和最终评级提取。
